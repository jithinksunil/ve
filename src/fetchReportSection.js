...
export async function fetchReportSection(section, options, openai, mcpClients) {
...
export async function fetchFullReport(options, openai, mcpClients, concurrency = 3) {
  ...
  for (let i = 0; i < sections.length; i += concurrency) {
    ...
    const data = await Promise.all(
      batch.map(async (section) => [section, await fetchReportSection(section, options, openai, mcpClients)])
    );
